...
//    }
    public boolean valuePossibleOnlyForOneSquare(char value, Square[] arr) { //Determines whether only one square in a given row, column, or box is expecting the given char value
        ...
        for (int i = 0; i < arr.length; i++) {
            if (arr[i].isValuePossible(value) == true) {
                countPossibleSquares += 1;
            }
        }
    ...
    //when my possibleValues change, only my row, column, and box are affected. This doesn't the possible values of the other squares on those row, column and box,
    //it only changes whether they now might be the only one that holds a missing value from that row, column, or box as their possible value.
    ...
    public void InitializePossibleNumbers() {
        for (int i = 0; i < this.board.length; i++) {
            for (int j = 0; j < this.board.length; j++) {
    ...
    public void Solve() {
        ...
        for (int i = 0; i < this.board.length; i++) {
            System.out.println(this.completionPercent());
//            if(this.completionPercent() == 1.0) return;
            for (int j = 0; j < this.board.length; j++) {
                Square square = this.board[i][j];
                if (square.value == '.') {
                    square.cleanPossibleNumbers();
